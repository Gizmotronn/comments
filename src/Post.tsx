...
export async function getPostDetails({params: { postId }, userContext, }: { params: { postId: string }; userContext: SupashipUserInfo; }): Promise<PostDetailData | undefined> {
    ...
    const postMap = (data as GetSinglePostWithCommentResponse[]).reduce(
        (acc, post) => { acc[post.id] = post; return acc; }, {} as Record<string, Post>
    );

    const post = postMap[postId];
    const comments = (data as GetSinglePostWithCommentResponse[]).filter((x) => x.id !== postId);
    if (!userContext.session?.user) { return { post, comments }};
...
export function PostView() {
    ...
    const [postDetailData, setPostDetailData] = useState<PostDetailData>({ post: null, comments: [], });
...
function CommentView ({ comment, myVotes, onVoteSuccess, }: { comment: Comment; myVotes: Record<string, "up" | "down" | undefined> | undefined; onVoteSuccess: () => void; }) {
...
function CreateComment({ parent, onCancel, onSuccess }: { parent: Comment | GetSinglePostWithCommentResponse; onCancel?: () => void; onSuccess: () => void; }) {
